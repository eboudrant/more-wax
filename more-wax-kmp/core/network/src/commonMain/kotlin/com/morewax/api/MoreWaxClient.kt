...
/**
 * HTTP client for the More'Wax Python server API. All methods are suspend functions returning typed
 * responses.
 */
class MoreWaxClient(val baseUrl: String = "http://localhost:8765") {
    ...
    suspend fun listCollection(): List<RecordDto> = http.get("/api/collection").body()

    suspend fun getRecord(id: Int): RecordDto = http.get("/api/collection/$id").body()
    ...
    suspend fun deleteRecord(id: Int): SimpleResponse = http.delete("/api/collection/$id").body()
    ...
    suspend fun getReleaseFull(id: String): ReleaseDto = http.get("/api/discogs/release/$id").body()

    suspend fun getReleasePrices(id: String): PricesDto = http.get("/api/discogs/prices/$id").body()
    ...
    suspend fun convertImage(base64Image: String): ConvertImageResponse =
        http.post("/api/convert-image") { setBody(mapOf("image" to base64Image)) }.body()

    suspend fun identifyCover(base64Image: String): IdentifyCoverResponse =
        http.post("/api/identify-cover") { setBody(mapOf("image" to base64Image)) }.body()
